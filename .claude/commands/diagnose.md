---
description: 启动 Desense 问题诊断流程(三要素模型 + 决策树 + 宽窄带判别)
...
| 特征 | 含义 | 排除/保留的干扰源类别 |
|---|---|---|
| **全频段 / 多信道 / 宽带** | 噪声带宽 ≥ 频段宽度 | ✓ 保留:PWM 驱动 / 电源 SSN / 模组本体辐射 / FPC 屏蔽泄露;✗ 排除:窄带谐波命中 |
| **仅 1~2 个信道 / 窄带** | 噪声集中在特定频点 | ✓ 保留:时钟谐波(MIPI/MCLK/DDR);✗ 排除:宽带 EMI 源 |
| **随 MIPI 配置变化** | 干扰频率追随分辨率/帧率 | 锁定 MIPI Clock/Data |
| **与 MIPI 配置无关** | 干扰频率稳定 | 转向 MCLK / OSC / 固定时钟源 |
...
**版本**:v2.0(2026-04-27 架构审计后重写,新增 Step 3 宽窄带判别、假设优先级排序、零成本验证路径表;巩固单一真相源地位)
