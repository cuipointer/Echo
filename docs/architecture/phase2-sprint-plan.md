...
Sprint 的 10 个任务中,除 T1.1(分支同步)/ T1.6(合入)/ T2.4(收尾)三个强串行节点外,其余 7 个任务都可通过并行压缩工期。
...
```
       ...
       ┌─────────────┼─────────────┐
       ▼             ▼             ▼
   T1.2 matrix   T1.3 方法论    T1.5 案例拆分
   (matrix.yaml) (bandwidth.md) (新 2 案例 + 改 triage)
       │             │             │
       │             ▼             │
       │         T1.4 模板+diagnose  │
       │         (依赖 T1.3 引用)   │
       ...
       └─────────────┼─────────────┘
                     ▼
               T1.6 合入 develop → 冷却 → main
                     │
       ┌─────────────┼─────────────┐
       ▼             ▼             ▼
   T2.1 LLB-07   T2.2 W24-03   T2.3 LHB-04
   (独立分支)    (独立分支)    (独立分支)
       │             │             │
       └─────────────┼─────────────┘
                     ▼
               T2.4 CHANGELOG + tag v2.1.0 + push
```
...
| 任务组 | 关系 | 说明 |
|---|---|---|
| T1.2 / T1.3 / T1.5 | 完全独立 | 改的文件无交集,可 3 路并行 |
| T1.4 | 弱依赖 T1.3 | 只依赖一行引用,可先写主体最后加引用 |
| T1.1 / T1.6 | 串行必经 | 分支同步必须先;合入必须最后 |

**Week 2 可并行度 ★★★★★**

3 个 SOP(LLB-07 / W24-03 / LHB-04)改的文件路径完全独立,纯并行。

### 9.3 三种并行执行方式

| 方式 | 场景 | 优势 | 代价 |
|---|---|---|---|
| **A. 单会话多工具并行** | Claude 一次消息内并行 Edit/Write 多文件 | context 共享、linter 一次过 | 输出长、失败排查复杂 |
| **B. Agent 委派** | 用 `general-purpose` Agent 并行启动多个 SOP 填充 | 时间实质并行 | context 不共享;prompt 要精细 |
| **C. Git Worktree 多工作树** | 多人协作(团队 3 人各填 1 个 SOP) | 物理隔离 | 单人不适用 |
...
|---|:---:|:---:|:---:|
| T1.1 | 0.5d | 0.5d | — |
| T1.2/T1.3/T1.5 并行 + T1.4 | 3.5d | 1.5d | -57% |
| T1.6 + 冷却 | 0.5d + 24h | 0.5d + 24h | — |
| T2.1/T2.2/T2.3 并行 | 4d | 1.5d | -63% |
| T2.4 | 1d | 0.5d | -50% |
| **总工期** | **10d** | **~5d (+1d 冷却)** | **-50%** |
...
| 新风险 | 应对 |
|---|---|
| 多文件一次改动,review 难 | 按任务组分 commit(T1.2 / T1.3 / T1.5 各一个),即使并行写也分开提交 |
| Agent 产出 SOP 质量参差 | Agent prompt 锁死模板路径、状态字段、状态版本(v0.9 待复测),主 Claude 收口审查 |
| linter 失败定位难 | 并行前跑一次基线,改完后一次跑对比 |
| 冷却期不能压缩 | 无法绕过,保留 24h(单人独立执行时可通过"累计多轮 commit 后再集中 review"变相替代) |

### 9.6 推荐执行节奏

**Week 1(Day 1 → Day 2.5)**:
- Day 1 上午:T1.1 分支同步(串行)
- Day 1 下午 ~ Day 2:一次性并行 T1.2 + T1.3 + T1.5(方式 A),分 3 个 commit
- Day 2 下午:T1.4(T1.3 完成后的增量变更)
- Day 2.5:T1.6 合 develop,冷却开始

**冷却 24h(Day 3)**

**Week 2(Day 4 → Day 5.5)**:
- Day 3 结束:合入 main
- Day 4:3 个 SOP 并行(方式 A 或 B)
- Day 5:review + 合入 develop + 合 main
- Day 5.5:T2.4 CHANGELOG + tag + push
...
| 场景 | 选择 |
|---|---|
| Phase 2 首次执行,风险未知 | 建议先用方式 A,Week 2 再视情况升级到 Agent |
| 3 个 SOP 模板化程度高 | 适合方式 B |
| 追求审查质量 | 保持串行 |
| 中断风险高 | 并行(任务更早完成) |
...
| 2026-05-06 | 新增第九章并行执行方案 | DAG 分析 + 3 种方式 + 5.5 天压缩时间表 |
